...
local function blend(color1, color2, alpha)
    ...
    color1 = type(color1) == 'number' and string.format('#%06x', color1) or
                 color1 or '#000000'
    ---@diagnostic disable-next-line: cast-local-type
    color2 = type(color2) == 'number' and string.format('#%06x', color2) or
                 color2 or '#000000'
    ...
    return '#' .. string.format('%02x', math.min(255, math.max(r, 0))) ..
               string.format('%02x', math.min(255, math.max(g, 0))) ..
               string.format('%02x', math.min(255, math.max(b, 0)))
end

---@diagnostic disable-next-line: param-type-mismatch
local function dim(color, n) return blend(color, '#000000', n) end
...
local WorkDir = {
    on_click = {
        callback = function() vim.cmd('Xplr') end,
...
local LSPActive = {
    ...
    on_click = {
        name = 'heirline_LSP',
        callback = function()
            vim.schedule(function() vim.cmd('LspInfo') end)
        end
    }
}
    ...
    {provider = '!['},
    {
        provider = function(self)
            ...
            return self.errors > 0 and
                       (self.error_icon .. self.errors .. self.separator)
        end,
        hl = {fg = 'diag_error'}
    },
    {
        provider = function(self)
            return self.warnings > 0 and
                       (self.warn_icon .. self.warnings .. self.separator)
        end,
        hl = {fg = 'diag_warn'}
    },
    {
        provider = function(self)
            return self.info > 0 and
                       (self.info_icon .. self.info .. self.separator)
        end,
        hl = {fg = 'diag_info'}
    },
    {
...
local Git = {
    ...
    {
        provider = function(self)
            return string.format('%s %s', git_icons.branch,
    ...
    {condition = function(self) return self.has_changes end, provider = '('},
    {
        provider = function(self)
            ...
            local separator = (self.status_dict.removed or 0) +
                                  ...
                                  ''
            return count > 0 and
                       (string.format('%s %s', git_icons.add, count) ..
                           separator)
        end,
        hl = {fg = 'git_add'}
    },
    {
        provider = function(self)
            ...
            local separator = (self.status_dict.added or 0) +
                                  ...
                                  ''
            return count > 0 and
                       (string.format('%s %s', git_icons.remove, count) ..
                           separator)
        end,
        hl = {fg = 'git_del'}
    },
...
local SearchCount = {
    ...
    provider = function(self)
        ...
        return string.format('%d/%d', search.current,
...
local DefaultStatusline = {
    ViMode, SearchCount, Space, Spell, WorkDir, FileNameBlock,
    {provider = '%<'}, Space, Git, Space, Diagnostics, Align, Navic, Align,
    LSPActive, Space, FileType,
    {flexible = 3, {Space, FileEncoding}, {provider = ''}}, Space, Ruler, Space,
    ScrollBar
}
...
local StatusLines = {
    ...
    fallthrough = false,
    {
        ...
        init = function() vim.opt_local.statusline = nil end
    },
    -- GitStatusline,
    SpecialStatusline,
    -- TerminalStatusline,
...
local CloseButton = {
    condition = function() return not vim.bo.modified end,
    -- update = 'BufEnter',
