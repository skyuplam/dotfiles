...
local WorkDir = {
  ...
  on_click={callback=function() vim.cmd('Xplr') end, name='heirline_workdir'},
  ...
  init=function(self)
    ...
    for i, d in ipairs(data) do
      local pos = self.enc(d.scope.start.line, d.scope.start.character,
                           self.winnr)
      local child = {
        {provider=d.icon, hl={fg=self.type_hl[d.type]}},
        {
...
--       local id = ''
--       ---@diagnostic disable-next-line: undefined-field
--       if vim.bo.filetype == 'toggleterm' then id = vim.b.toggle_number end
--       return ' ' .. (id or 'Exited')
--     end,
...
local WinBar = {
  ...
  utils.surround({'', ''}, 'bright_bg', {
    hl=function()
      if conditions.is_not_active() then return {fg='gray', force=true} end
    end,
